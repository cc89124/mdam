...
Usage
-----
    python -m qv_bench --min-q 6 --max-q 26 --repeats 3
    python -m qv_bench --qubits 10,14,18,22 --simulators clifft,qiskit
"""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--output",
        type=str,
        default=str(_PROJECT_ROOT / "results.csv"),
        help="Output CSV path (default: results.csv in the workspace root).",
    )
    p.add_argument(
        "--seed",
        type=int,
    ...
    p.add_argument(
        ...
        type=int,
        default=1,
        help="Thread count to expose to simulator backends (default: 1).",
    )
    return p
...
def _make_env(mem_limit_gb: float, threads: int) -> dict[str, str]:
    """Build an environment dict for the subprocess worker."""
    env = os.environ.copy()
    env["OMP_NUM_THREADS"] = str(threads)
    env["MKL_NUM_THREADS"] = str(threads)
    env["OPENBLAS_NUM_THREADS"] = str(threads)
    env["CLIFFT_BENCH_THREADS"] = str(threads)
...
def _run_worker(
    ...
    for ln in reversed(stdout_lines):
        try:
            ...
            if proc.returncode != 0:
                data.setdefault("status", _classify_failure(proc.returncode, proc.stderr))
            else:
    ...
    # No parseable JSON — fall back to classifying from returncode/stderr.
    if proc.returncode != 0:
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    env = _make_env(args.mem_limit_gb, args.threads)
