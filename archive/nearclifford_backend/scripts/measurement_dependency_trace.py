...
def _flush_core(self, qx, qz):
    ...
    pend_vals = list(self.pending.values())          # uid map -> ordered snapshot
...
def trace_circuit(circuit):
    ...
    # drop_dead=False: this trace MEASURES the never-flushed rotations by reading the
    # leftover `pending` at circuit end, so it must keep them resident (pruning them is
    # exactly what dead-drop does -- but here we want to count them).
    # structure_once=False: it monkeypatches _flush_core and re-derives the core with a live
    # _core_indices scan every measurement, so it needs the dynamic path, not the fast lookup.
    be = NearCliffordBackend(block=True, drop_dead=False, structure_once=False)
    TRACE.clear()
