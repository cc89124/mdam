...
(R_X / R_Y) coherent noise, one plot per circuit + a grid.  y = active-state size in qubits
(= log2 dense-equivalent dimension).  Reads bounded_<circuit>_per_step.csv (n_active = Clifft
active rank k; bounded_*_qubits).  Only the feasible d=3 circuits are traced (d=5 off-axis
exceeds the 1-GiB ceiling -- see BOUNDED_RXRY_SUMMARY.md)."""
import os, csv
...
def draw(ax, c):
    ...
    ax.fill_between(step, res, k, color=CL, alpha=0.12)          # the saving (clifft over bounded)
...
plt.close(fig)
print("WROTE", OUT + "/bounded_vs_clifft_<circuit>_qubits.png (x4) + bounded_vs_clifft_ALL_qubits.png")
