"""Build DETAILED_TABLE.xlsx (+ .md) for the LATEST live clifft_axis_bounded backend.
...
(`bounded_<circuit>_per_step.csv`), i.e. the dense-free single-frame backend's OWN run.

Three sheets:
  1. Transient vs Resident  -- PEAK active-state, Clifft 2^k vs bounded transient / resident.
...
def load(circ):
    ...
    f = f"{OUT}/bounded_{circ}_per_step.csv"
    na, tr, res = [], [], []
    for r in csv.DictReader(open(f)):
        na.append(int(r['n_active']))
        tr.append(int(r['bounded_transient_qubits']))
        res.append(int(r['bounded_resident_qubits']))
...
hdr = ['circuit', 'Clifft PEAK 2^k', 'bounded transient 2^b', 'transient vs Clifft',
       'bounded resident 2^b', 'resident vs Clifft']
...
def peak_sum_sheet(title, fmt, val):
    ...
    Columns: circuit | PEAK[Clifft, bounded transient, x, bounded resident, x] |
                       SUM [Clifft, bounded transient, x, bounded resident, x]."""
    ...
    sub = ['circuit',
           'Clifft', 'bounded transient', 'x', 'bounded resident', 'x',
           'Clifft', 'bounded transient', 'x', 'bounded resident', 'x']
...
wb.save(f"{OUT}/DETAILED_TABLE_RXRY.xlsx")
print("WROTE", f"{OUT}/DETAILED_TABLE_RXRY.xlsx")
...
lines = []
lines.append("# DETAILED TABLE — Clifft baseline vs live clifft_axis_bounded backend\n")
lines.append("All bounded numbers are read from the per-step traces the "
             "`CliftAxisBoundedNearClifford` engine emits during its OWN run "
             "(`bounded_<circuit>_per_step.csv`) — no TTN, no block, no Clifft state, no forced "
             ...
             "(poly, same basis as Clifft's `2^k`).  `transient` = peak materialized magic rank during a "
...
r = [['circuit', 'Clifft PEAK 2^k', 'bounded transient 2^b', 'transient vs Clifft',
      'bounded resident 2^b', 'resident vs Clifft']]
for c in CIRCS:
    s = S[c]
...
r = [['circuit', 'Clifft PEAK', 'bounded tr. PEAK', 'tr x', 'bounded res. PEAK', 'res x',
      'Clifft SUM', 'bounded tr. SUM', 'tr x', 'bounded res. SUM', 'res x']]
for c in CIRCS:
    s = S[c]
...
open(f"{OUT}/DETAILED_TABLE_RXRY.md", "w").write('\n'.join(lines))
print("WROTE", f"{OUT}/DETAILED_TABLE_RXRY.md")
