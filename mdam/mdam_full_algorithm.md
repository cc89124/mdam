...
> Markdown 수식 호환성을 위해 display math는 `$$...$$`, inline math는 `$...$` 형식으로 작성했다.
...
$\mathrm{fb}_w<\mathrm{fb}_{be}$면 LEAN이 **지금** AUTH보다 빠르다 — 판정 끝, 유지. ($T_{\mathrm{slow}}<\hat T_{\mathrm{auth}}$인 회로는 $\mathrm{fb}_{be}>1$이라 어떤 miss율에서도 유지된다.)
...
- 현재 fb를 동결한 비용 $\mathrm{fb}_w T_{\mathrm{slow}}+(1-\mathrm{fb}_w)T_{\mathrm{walk}}<\hat T_{\mathrm{auth}}$이
