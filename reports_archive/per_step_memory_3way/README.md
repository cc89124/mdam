...
2. **The exponential state never loses.** Compare `16·2^block` vs `16·2^k` (the dotted
   ...
   every circuit**. Example: `coherent_d3_r1` has NC magic `2^0` (no magic at all), so its
   dense state is infinitely smaller than Clifft's `2^5`.
3. **The only remaining `<1×` total cells are genuine, not artifacts.** After dead-rotation
   pruning (below) the coherent families win on *total* footprint too (`coherent_d3_r1`
   PEAK `512 B → 48 B`, **10.7×**; `coherent_d5_r1` **390×**; `coherent_d5_r5` **1949×**).
   The cells still `<1×` are `cultivation_d3` (0.62× PEAK) and `cultivation_d5` (0.99× PEAK):
   those circuits have **0% never-flushed rotations** — every rotation is genuinely consumed,
   so the metadata is real working state, and a small all-magic register plus the `O(n²)`
   tableau genuinely sits near Clifft's tiny dense model. That is the honest all-magic
   near-Clifford regime, not a bookkeeping artifact.
...
**Dead-rotation pruning** (`NearCliffordBackend(drop_dead=True)`, default ON) goes further:
a rotation that is *never* flushed — never enters any measurement's anticommutation core,
so it commutes with every measured Pauli forever — affects no record bit, and is dropped
from `pending` outright (record-bit-identical). This collapses the end-of-circuit floor
that used to never decrease: `coherent_d3_r1`'s 25 never-flushed rotations (its entire
pending) vanish, leaving only the tableau. Circuits with 0% never-flushed (cultivation,
distillation) are unchanged. The pruned fraction equals the `never_ratio` in
`reports/measurement_dependency_trace/`.
