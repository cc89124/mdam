...
class _DemError:
    ...
    def from_error_instruction(instruction: stim.DemInstruction) -> "_DemError":
        ...
        return _DemError(
            p=p, det_set=frozenset(gen.xor_sorted(det_list)), obs_mask=obs_mask
        )
    ...
    def from_task(task: sinter.Task) -> "CompiledDesaturationSampler":
        ...
        for err in errors:
            ...
            if len(err.det_set) == 3:
                ...
                and len(bases) == 1
                and (
                    colors == {"r", "g"} or colors == {"r", "b"} or colors == {"b", "g"}
                )
            ):
                a, b = err.det_set
                virtual_pair_nodes.add(frozenset([a, b]))
        ...
        for pair in sorted(virtual_pair_nodes, key=lambda e: tuple(sorted(e))):
            ...
            if a != -1 and b != -1:
                det_coords[k] = [
                    (x + y) / 2 for x, y in list(zip(det_coords[a], det_coords[b]))[:3]
                ]
            else:
                c = a if a != -1 else b
        ...
        for k in range(num_dets + len(pair2virtual)):
            matchable_dem.append(
                "detector", det_coords[k], [stim.target_relative_detector_id(k)]
            )
        for err in errors:
            ...
            if len(err.det_set) == 2 and err.det_set in virtual_pair_nodes:
                ...
                len(err.det_set) == 3
                and len(bases) == 1
                and colors == collections.Counter("rgb")
            ):
                ...
                for solo in err.det_set:
                    virtual_err = _DemError(
                        ...
                        obs_mask=err.obs_mask,
                        det_set=frozenset(
                            [solo, pair2virtual[err.det_set ^ frozenset([solo])]]
                        ),
                    )
                    matchable_dem.append(virtual_err.to_instruction())

            elif len(err.det_set) <= 2 and (
                bases.keys() == {"X"} or bases.keys() == {"Z"}
            ):
                ...
                matchable_dem.append(
                    _DemError.to_separated_instruction(
                        [
                            x_part,
                            z_part,
                        ]
        ...
        clipped_dem = clipped_matchable_dem(
            matchable_dem, postselected_detectors_hidden_from_matcher
        )
            ...
            stim.DemInstruction(
                ...
                [
                    stim.target_relative_detector_id(
                        clipped_dem_with_det_for_obs.num_detectors - 1
                    )
                ],
            )
        )
        return CompiledDesaturationSampler(
            ...
            postselected_detectors=frozenset(
                postselected_detectors_hidden_from_matcher
                | postselected_detectors_visible_to_matcher
            ),
