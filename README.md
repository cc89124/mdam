...
**전환(LEAN → AUTH)은 아래 4개 중 하나라도 참이면 발화**한다(기본값 기준).

1. **fine 메모리 게이트** *(64샷마다 체크)* — 최근 miss율 `> 0.95` **AND** 캐시 바이트 `> 512 MB`.
   miss율을 먼저(O(1)) 보므로, 캐시가 *이득을 내는* 회로는 바이트 스캔 자체를 돌리지 않는다.
   *heavy-core 비포화 회로에서 발화* (`coherent_d5_r5`: `maxM=12`, ~3.75 MB/샷) → **AUTH@191**.
...
3. **count cap** *(OOM 백스톱)* — lean 노드 `> 1e6` 또는 엣지 `> 4e6`.
4. **보수적 cost 경로** — 100k샷 horizon을 넘고, node-rate가 floor 위에 있고, window LEAN 비용 `>`
   fallback 비용이 3 window 연속 지속.

각 게이트는 **서로 다른 LEAN 승자를 지키는 역할**을 한다: 메모리 게이트는 작은-코어 회로를, miss율
게이트는 캐시-hit이 있는 승자(`cultivation_d5`, ~50% hit)를, `magic_ever` 게이트는 magic 회로를, 게이트 4의
node-rate 감쇠는 느리게 포화하는 승자(`cultivation_d3`, ~100k샷에서 포화)를 각각 보호한다. 순효과:
**비포화·캐시-낭비 회로만 AUTH로 밀고, 모든 LEAN 승자는 LEAN에 그대로 둔다.**
...
*clifft*는 **기준선으로만 쓰는 외부 참조 시뮬레이터**다 — MDAM 내부에서는 절대 참조하지 않는다. 비율은
가감 없이 보고하며 **손해(`< 1×`)도 숨기지 않는다**. 전체 데이터·열 설명:
...
| cultivation_d5 | 10 | 9 | magic, 캐시 비포화 | 0.43× | 0.72× | **0.62×** | LEAN |
...
- 일부 회로는 기준선 대비 **순손해(`< 1×`)** 다. 그래도 adaptive는 가장 덜 나쁜 경로를 고르고 bit-exact를
  유지한다.
